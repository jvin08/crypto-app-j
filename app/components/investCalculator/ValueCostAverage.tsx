...
const ValueCostAverage = ({coin}:{coin: string}) => {
  ...
  const calculateVCA = () => {
    allSetUp && setShowSpentAmount(true);
  };
    ...
    <div className="w-full mb-8 sm:relative">
      <div className="flex sm:flex-col relative text-base mb-4">
        ...
        <DatePicker 
          ...
          info={info.endDate}
        />
        <p className={clsx("sm:hidden py-2 h-9 w-[83px] text-center ml-auto rounded-lg text-sm font-semibold",{
          "bg-cryptodark-350 text-cryptoblue-650": darkmode,
          "bg-cryptoblue-350 text-cryptoblue-660": !darkmode,
        })}>Q-ty</p>
      </div>
      ...
      <div className={clsx("text-base flex rounded-xl px-8 py-6 mb-8",{
        ...
        <Table 
          ...
          days={days}
          query={query}
          rowThreeName="Grow rate per interval, %"
        />
      </div>
