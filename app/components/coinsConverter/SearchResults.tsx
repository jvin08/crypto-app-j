...
const DropdownSearch = ({query, toggleHidden, toggleSearch, clearSearch, buyOrSell} : {query: string, toggleHidden: ()=>void, toggleSearch: ()=>void, clearSearch: ()=>void, buyOrSell: string}) => {
  ...
  },[toggleHidden, clearSearch, toggleSearch]);
  const showBorder = coinsForRender !== undefined && coinsForRender.length > 0;
  return (
    <div className={clsx("box-border text-sm w-[14.62rem] left-0 top-10 absolute z-50 rounded",{
