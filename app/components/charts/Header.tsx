...
export const Header = ({dataOne, price, compare, priceDate } : {dataOne: string, price: number, compare: boolean, priceDate: string}) => {
  ...
  return (
    <div className="sm:flex sm:justify-between">
      {compare ? <p className={clsx("text-2xl sm:text-base font-bold tabular-nums", {
      ...
      <div>
        ...
        })}>.</p> : <p className={clsx("text-2xl sm:text-xl sm:font-normal font-bold sm:text-right",{
          "text-cryptodark-100": darkmode,
          "text-cryptodark-400": !darkmode,
        })}>{coinLastPrice}</p>}
        {compare ? <p className={clsx("text-sm sm:text-xs",{
          "text-cryptoblue-100": !darkmode,
          "text-cryptodark-350": darkmode,
        })}>.</p> : <p className={clsx("text-sm sm:text-xs tabular-nums sm:text-right",{
          "text-cryptodark-100 sm:text-cryptodark-510": darkmode,
          "text-cryptodark-400": !darkmode,
        })}>{dateNow}</p>}
      </div>
    </div>             
...
export const VolumeHeader = ({volume, compare, volumeDate } : {volume: number[][], compare: boolean, volumeDate: number}) => {
  ...
  return (
    <div className={clsx("sm:flex sm:justify-between",{
      "text-cryptodark-400": !darkmode,
    })}>
      {compare ? <p className={clsx("text-2xl sm:text-base font-bold", {
        "text-cryptodark-100": darkmode,
      ...
      <div>
        {compare ? <p className={clsx("text-sm",{
          "text-cryptodark-550": darkmode,
        })}>{dateNow}</p> : <p className={clsx("text-2xl sm:text-xl sm:font-normal font-bold sm:text-right",{
          "text-cryptodark-100": darkmode,
        })}>{volumeToShow}</p>}
        {compare ? <p className={clsx("text-xl sm:text-xs",{
          "text-cryptodark-350": darkmode,
          "text-cryptodark-100": !darkmode,
        })}>.</p> : <p className={clsx("text-sm sm:text-xs tabular-nums sm:text-right",{
          "text-cryptodark-550 sm:text-cryptodark-510": darkmode,
        })}>{dateNow}</p>}
      </div>
    </div>             
  );
